...
class Algorithm:
    ...
    def __init__(self,
        ...
        if client_num is None and client_list is not None:
            ...
            # Debug print to verify data assignment
            for client in client_list:
                print(f"Client {client.id} training data batches: {len(client.local_training_data) if client.local_training_data is not None else 'None'}")
                print(f"Client {client.id} test data batches: {len(client.local_test_data) if client.local_test_data is not None else 'None'}")
    ...
    def terminated(self):
        ...
        # Track client participation frequency
        if not hasattr(self, 'client_participation_count'):
            self.client_participation_count = {client.id: 0 for client in self.client_list}
        for client in self.online_client_list:
            self.client_participation_count[client.id] += 1
