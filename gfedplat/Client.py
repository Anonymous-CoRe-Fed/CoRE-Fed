...
class Client:
    ...
    def test(self, test_module):
        ...
        # Removed debug prints as per user request
        # print(f"Client {self.id} test method called.")  # Debug print to confirm test call
        ...
        with torch.no_grad():
            ...
            for metric in self.metric_list:
                ...
                if metric.name == 'correct':
                    accuracy = 100 * metric_dict['correct'] / self.local_test_number
                    self.metric_history['test_accuracy'].append(accuracy)
                    # print(f"Client {self.id} test accuracy calculated: {accuracy}")  # Debug print for accuracy
